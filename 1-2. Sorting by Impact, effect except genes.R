...
for (i in 1:src_file_lnc){
  ...
  assign(paste0("Col_", i),filter(get(paste0("NGS_",i)), Putative_Impact != "LOW",
                                  Putative_Impact != "MODIFIER"))
  
  Genename <- get(paste0("Col_",i))[!duplicated(get(paste0("Col_",i))$POS),1:29]#중복된 position 제거
  
  names(Genename)[1] <- c("CHROM")
  names(Genename)[names(Genename) == "dbSNP142_ID"] <- c("ID")
  names(Genename)[names(Genename) == "Putative_Impact"] <- c("Impact")
